...
class XtExchange(ExchangePyBase):
    ...
    async def _place_order(self,
        ...
        try:
            ...
            if "result" not in order_result or order_result["result"] is None:
                if "mc" in order_result and "clientOrderId error" in order_result["mc"]:
                    # Generate a new client order ID and retry
                    new_order_id = f"{self.client_order_id_prefix}{get_tracking_nonce()}"
                    self.logger().warning(f"ClientOrderId was rejected. Retrying with new ID: {new_order_id}")
                    api_params["clientOrderId"] = new_order_id
                    order_result = await self._api_post(
                        path_url=CONSTANTS.ORDER_PATH_URL,
                        data=api_params,
                        is_auth_required=True
                    ...
                    if "result" not in order_result or order_result["result"] is None:
                        raise ValueError(f"Order placement failed after retry: {order_result}")
                else:
                    raise ValueError(f"Order result is missing or null: {order_result}")
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        # for rule in filter(xt_utils.is_exchange_information_valid, trading_pair_rules):
        #     try:
        #         # Split the trading pair correctly by the underscore ("_") separating base and quote
        #         raw_symbol = rule.get("symbol")
        #         base, quote = raw_symbol.split("_")
        #         trading_pair = f"{base}-{quote}"  # Format as expected by Hummingbot (base-quote)
                ...
                if event_type == "order":
                    ...
                    if tracked_order is not None:
                        ...
                        order_update = OrderUpdate(
                            ...
                            update_timestamp=order_update.get("t", self.current_timestamp) * 1e-3,
    ...
    async def _request_order_status(self, tracked_order: InFlightOrder) -> OrderUpdate:
        ...
        try:
            self.logger().debug(f"Requesting order status for {client_order_id}")
            response = await self._api_get(
                path_url=CONSTANTS.ORDER_PATH_URL,
                params={
                    "orderId": int(exchange_order_id),
            ...
            self.logger().debug(f"Order status response: {response}")

            if "result" not in response or response["result"] is None:
                raise ValueError(f"Order status request failed. Response: {response}")
            ...
            update_timestamp = updated_order_data.get("updatedTime")
            if update_timestamp is None:
                update_timestamp = updated_order_data.get("time")
                self.logger().debug(f"Using 'time' field for timestamp: {update_timestamp}")

            if update_timestamp is None:
                self.logger().warning(f"No timestamp in order status: {updated_order_data}")
                update_timestamp = self.current_timestamp
            else:
                try:
                    update_timestamp = float(update_timestamp) * 1e-3
                except (TypeError, ValueError):
                    self.logger().warning(f"Invalid timestamp in order status: {update_timestamp}")
                    update_timestamp = self.current_timestamp
            ...
            self.logger().network(
                f"Error fetching status update for the order {client_order_id}: {str(e)}",
                app_warning_msg=f"Failed to fetch status update for the order {client_order_id}."
            )
            self.logger().error(f"Order status request failed. Response: {response}", exc_info=True)
            raise e
